...
- `npm run seed:seo` materialises the defaults into the database (`scripts/seed-seo.mts`). Idempotent and non-destructive by default because it runs through the same `mergeSeo()`; `--force` resets to defaults, `--dry-run` previews. Adding a field to `SEO_DEFAULTS` needs no migration — the fallback covers existing installs, and the seed fills it in on the next run.
